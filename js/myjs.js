...
*/
  $(document).ready(function(){
           $('#dateDiv').timer({format: "yy-mm-dd"});
           $('#timeDiv').timer({format: "HH:MM"});
...
// 开始菜单
    ...
    $(".shutdown-panel").click(function (){
      $(".download-window").show();
    });

    $(".shutdown-button").click(function (){
      $(".shutdown-panel-div").show();
    });

    $(".go-around").click(function (){
      $("#welcome").hide();
    });


    $("#go-baby").click(function (){
...
// 绑定鼠标事件
  $(function(){
   $(document).bind("click",function(e){
    var target  = $(e.target);
    if((target.closest(".shutdown-button").length == 0) ){
         $(".shutdown-panel-div").hide();
    }
    if((target.closest("#go-baby").length == 0) && (target.closest("#tabs-2").length == 0)){
		if(target.closest(".sidetab-menu-title").length == 0){
         $("#start-menu").hide();
		}
    }
   })
  ...
  function over(event) {
    var tag = event.srcElement || event.target;//IE有srcElement属性，而在Firefox中为target属性。
    var realTag;
    if (tag.tagName == "A") {
      realTag = tag.parentNode;
    } else if (tag.tagName == "SPAN" || tag.tagName == "IMG") {
      realTag = tag.parentNode.parentNode;
    } else {
      realTag = tag;
    }
    if (realTag.tagName == "LI") {
      var tagParentNode = realTag.parentNode;//事件源的父节点 ul 
      /*           alert(realTag.tagName);
       alert(tagParentNode.tagName);*/
       
      if(realTag.id!="tabs-button1"){
        $(".tabs-test").css("left","365px");
      }

      for (var i = 0; i < tagParentNode.getElementsByTagName("li").length; i++) {
        tagParentNode.getElementsByTagName("li")[i].className = "tabs-noselected";//去掉所有选项卡的单项的样式  
        tagParentNode.getElementsByTagName("li")[i]
            .getElementsByTagName("a")[0].className = "tabs-text-noselected";//去掉所有选项卡的单项的样式  
      }
      realTag.className = "tabs-selected";//为触发onmouseover事件的事件选项卡添加样式 
      realTag.getElementsByTagName("a")[0].className = "tabs-text-selected";//为触发onmouseover事件的事件选项卡添加样式   
    }
  };
  //加载页面
